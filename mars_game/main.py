...
def draw_storage_window(screen, inventory, storage_inventory, storage_side, selected_slot, storage_num=1):
    panel_width = 760
    panel_height = 420
    panel_x = (WIDTH - panel_width) // 2
    panel_y = (HEIGHT - panel_height) // 2
    ...
    right_title = font.render(f"STORAGE {storage_num}", True, UI_TEXT)
    screen.blit(left_title, (panel_x + 115, panel_y + 25))
    screen.blit(right_title, (panel_x + 480, panel_y + 25))
...
def get_active_storage(current_storage, storage_inventory, storage_inventory2):
    if current_storage == 2:
        return storage_inventory2
...
def draw_computer_menu(screen, computer_menu, computer_selected, inventory=None, storage_inventory=None,
                       rover_speed_level=0, base_storage_level=0, upgrade_costs=None, player_speed_level=0):
    ...
    font = pygame.font.Font("mars_game/fonts/PressStart2P-Regular.ttf", 16)
    small_font = pygame.font.Font("mars_game/fonts/PressStart2P-Regular.ttf", 11)
    ...
    if computer_menu == "main":
        ...
        start_y = panel_y + 110
        costs = upgrade_costs.get("player_pickup", [2, 4, 6])
        level = player_speed_level
        max_level = len(costs)

        y = start_y
        if 0 == computer_selected:
            pygame.draw.rect(screen, UI_SELECTED, (panel_x + 55, y - 12, panel_width - 110, 38), 3)
            color = UI_SELECTED
        else:
            color = UI_TEXT_DIM
        if level >= max_level:
            label = "PICKUP SPD  [MAX]"
        else:
            label = f"PICKUP SPD LVL{level}/{max_level} CST:{costs[level]}"
        text = font.render(label, True, color)
        screen.blit(text, (panel_x + 80, y))

        for i, option in enumerate(["HP", "ARMOR"]):
            y = start_y + (i + 1) * 55
            if (i + 1) == computer_selected:
                pygame.draw.rect(screen, (60, 60, 80), (panel_x + 55, y - 12, panel_width - 110, 38), 3)
                color = (150, 150, 170)
            else:
                color = (80, 80, 100)
            text = font.render(option, True, color)
            screen.blit(text, (panel_x + 80, y))
            lock_text = small_font.render("[LOCKED]", True, (180, 80, 80))
            screen.blit(lock_text, (panel_x + 390, y))

    elif computer_menu == "rover":
        start_y = panel_y + 110
        costs = upgrade_costs.get("rover_speed", [2, 4, 6])
        level = rover_speed_level
        max_level = len(costs)
        ...
        if level >= max_level:
            ...
            label = f"SPEED  LVL {level}/{max_level}  COST: {costs[level]} DET"
        text = font.render(label, True, color)
        screen.blit(text, (panel_x + 80, y))

        for i, option in enumerate(["ARMOR", "BATTERY"]):
            y = start_y + (i + 1) * 55
            if (i + 1) == computer_selected:
                pygame.draw.rect(screen, (60, 60, 80), (panel_x + 55, y - 12, panel_width - 110, 38), 3)
                color = (150, 150, 170)
        ...
        start_y = panel_y + 110
        costs = upgrade_costs.get("base_storage", [3, 5])
        level = base_storage_level
        max_level = len(costs)
        ...
        if level >= max_level:
            ...
            label = f"STORAGE LVL {level}/{max_level} COST:{costs[level]} DET"
        text = font.render(label, True, color)
        screen.blit(text, (panel_x + 80, y))

        for i, option in enumerate(["ARMOR", "ENERGY"]):
            y = start_y + (i + 1) * 55
            if (i + 1) == computer_selected:
                pygame.draw.rect(screen, (60, 60, 80), (panel_x + 55, y - 12, panel_width - 110, 38), 3)
                color = (150, 150, 170)
...
def draw_home_computer(screen, home_info_selected, rover_inside_base, inventory, storage_inventory, resources):
    ...
    if home_info_selected == 0:
        ...
        lines = ["HOME STATUS: ONLINE", "OXYGEN: NORMAL", "ENERGY: NORMAL"]
    elif home_info_selected == 2:
        lines = ["GREENHOUSE STATUS: ONLINE", "PLANTS: STABLE", "WATER: NORMAL"]
    else:
        rover_place = "IN BASE" if rover_inside_base else "ON MARS"
        lines = ["ROVER STATUS: ONLINE", f"LOCATION: {rover_place}", f"MARS RESOURCES: {len(inventory)}"]
    for i, line in enumerate(lines):
        text = small_font.render(line, True, UI_TEXT)
        screen.blit(text, (panel_x + 90, info_y + i * 45))


def draw_greenhouse_menu(screen, greenhouse_selected, crop_recipes, growing_crop, growing_timer, growing_ready):
    panel_width = 620
    panel_height = 420
    panel_x = (WIDTH - panel_width) // 2
    panel_y = (HEIGHT - panel_height) // 2
    ...
    for i, key in enumerate(recipe_keys):
        ...
        if i == greenhouse_selected:
            color = UI_SELECTED
            pygame.draw.rect(screen, UI_SELECTED, (panel_x + 40, y - 10, 520, 38), 3)
        text = font.render(f"{recipe['title']} - {recipe['cost']} DETAILS", True, color)
...
def draw_pause_menu(screen, selected):
    ...
    pygame.draw.rect(screen, UI_SHADOW, (px + 8, py + 8, panel_w, panel_h))
    pygame.draw.rect(screen, UI_BG, (px, py, panel_w, panel_h))
    pygame.draw.rect(screen, UI_BORDER, (px, py, panel_w, panel_h), 5)

    font = pygame.font.Font("mars_game/fonts/PressStart2P-Regular.ttf", 16)
    small = pygame.font.Font("mars_game/fonts/PressStart2P-Regular.ttf", 13)

    title = font.render("PAUSED", True, UI_TEXT)
    screen.blit(title, (px + panel_w // 2 - title.get_width() // 2, py + 30))

    options = ["CONTINUE", "SETTINGS", "MAIN MENU"]
    for i, opt in enumerate(options):
        y = py + 100 + i * 60
        if i == selected:
            pygame.draw.rect(screen, UI_SELECTED, (px + 40, y - 10, panel_w - 80, 38), 3)
            color = UI_SELECTED
        else:
            color = UI_TEXT_DIM
        text = small.render(opt, True, color)
        screen.blit(text, (px + panel_w // 2 - text.get_width() // 2, y))
def draw_pause_settings(screen, selected, show_fps, music_volume):
    overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
    ...
    title = font.render("PAUSE SETTINGS", True, UI_TEXT)
    screen.blit(title, (px + 90, py + 30))

    options = [
        f"SHOW FPS: {'ON' if show_fps else 'OFF'}",
        f"MUSIC: {music_volume}",
        "BACK"
    ]
    ...
    for i, option in enumerate(options):
        ...
        if i == selected:
            pygame.draw.rect(screen, UI_SELECTED,
                             (px + 40, y - 10, panel_w - 80, 38), 3)
            color = UI_SELECTED
        else:
            color = UI_TEXT_DIM

        text = small.render(option, True, color)
        screen.blit(text, (px + 60, y))
def main():
    pygame.init()
    ...
    camera_y = 0
    try:
        sound_pickup = pygame.mixer.Sound("mars_game/sounds/coin.wav")
        sound_pickup.set_volume(1)
    except:
        sound_pickup = None
    settings_from = "menu"
    ...
    show_pause = False
    pause_selected = 0
    sound_pickup.set_volume(0.7)

    vishka = MapObject(690, 600, "mars_game/img/vishka.png", (200, 200))
    generator = MapObject(160, 800, "mars_game/img/generator.png", (90, 100))
    map_objects = [vishka,generator]
    ...
    base_walls = [
        pygame.Rect(95, 240, 840, 20),
        pygame.Rect(95, 920, 840, 20),
        pygame.Rect(140, 95, 20, 820),
        pygame.Rect(860, 95, 20, 820),
    ...
    resource_types = [
        {"name": "mars_stone",     "image": "mars_game/img/resurs1.png",   "size": (64, 64),   "shans": 85, "pickup_time": 1},
        {"name": "big_mars_stone", "image": "mars_game/img/resurs(2).png", "size": (160, 160), "shans": 20, "pickup_time": 3},
    ]

    detail_item = {"name": "detail", "image": "mars_game/img/detail1 (1).png"}

    crop_recipes = {
        "potato": {"title": "POTATO", "cost": 1, "time": 5 * FPS, "item": {"name": "potato", "image": "mars_game/img/potato.png"}},
        "corn":   {"title": "CORN",   "cost": 2, "time": 8 * FPS, "item": {"name": "corn",   "image": "mars_game/img/corn.png"}},
        "carrot": {"title": "CARROT", "cost": 1, "time": 6 * FPS, "item": {"name": "carrot", "image": "mars_game/img/carrot.png"}},
    }
    ...
    while True:
            ...
            if event.type == pygame.KEYDOWN:
                if show_pause_settings:
                    if event.key == pygame.K_ESCAPE:
                        ...
                        pause_settings_selected = (pause_settings_selected - 1) % 3

                    elif event.key == pygame.K_DOWN:
                        pause_settings_selected = (pause_settings_selected + 1) % 3

                    elif event.key == pygame.K_LEFT:
                        if pause_settings_selected == 1:
                        ...
                        if pause_settings_selected == 0:
                            show_fps = not show_fps

                        elif pause_settings_selected == 2:
                            show_pause_settings = False
                            show_pause = True
                ...
                if show_pause:
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if pause_selected == 0:
                            ...
                            try:
                                ...
                                pygame.mixer.music.load("mars_game/sounds/menu_music.ogg")
                    ...
                    continue
                if event.key == pygame.K_ESCAPE and scene not in ("menu", "settings", "about"):
                    if not show_computer and not show_storage and not show_inventory and not show_home_computer and not show_greenhouse_menu:
                        show_pause = True
                        pause_selected = 0
                ...
                if show_greenhouse_menu:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        greenhouse_selected = (greenhouse_selected - 1) % len(recipe_keys)
                    elif event.key == pygame.K_DOWN:
                        greenhouse_selected = (greenhouse_selected + 1) % len(recipe_keys)
                    elif event.key == pygame.K_RETURN:
                        if growing_crop is None or growing_ready:
                            ...
                            total_details = count_item(inventory, "detail") + count_item(storage_inventory, "detail")
                            if total_details >= recipe["cost"]:
                                remove_details(inventory, storage_inventory, recipe["cost"])
                ...
                if show_home_computer:
                    if event.key == pygame.K_ESCAPE:
                        ...
                        home_info_selected = (home_info_selected - 1) % 4
                    elif event.key == pygame.K_RIGHT:
                        home_info_selected = (home_info_selected + 1) % 4
                    continue
                ...
                if show_computer:
                    ...
                    if computer_menu == "player":
                        ...
                        current_options_count = max(1, min(15, len(inventory) + len(storage_inventory)))
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        computer_selected = (computer_selected - 1) % current_options_count
                    elif event.key == pygame.K_DOWN:
                        computer_selected = (computer_selected + 1) % current_options_count
                        ...
                        if computer_menu == "main":
                            ...
                            if computer_selected == 0:
                                costs = upgrade_costs["player_pickup"]
                                if player_speed_level < len(costs):
                                    cost = costs[player_speed_level]
                                    total = count_item(inventory, "detail") + count_item(storage_inventory, "detail")
                                    if total >= cost:
                                        remove_details(inventory, storage_inventory, cost)
                                        player_speed_level += 1

                        elif computer_menu == "rover":
                            if computer_selected == 0:
                                costs = upgrade_costs["rover_speed"]
                                if rover_speed_level < len(costs):
                                    cost = costs[rover_speed_level]
                                    total = count_item(inventory, "detail") + count_item(storage_inventory, "detail")
                                    if total >= cost:
                                        remove_details(inventory, storage_inventory, cost)
                                        rover_speed_level += 1
                                        rover.speed = rover.base_speed + rover_speed_level * 1.5

                        elif computer_menu == "base":
                            if computer_selected == 0:
                                costs = upgrade_costs["base_storage"]
                                if base_storage_level < len(costs):
                                    cost = costs[base_storage_level]
                                    total = count_item(inventory, "detail") + count_item(storage_inventory, "detail")
                                    if total >= cost:
                                        remove_details(inventory, storage_inventory, cost)
                                        base_storage_level += 1
                            ...
                            if all_count > 0:
                                ...
                                allowed_resources = {"mars_stone": 1, "big_mars_stone": 3}
                                if item["name"] in allowed_resources:
                                    for _ in range(allowed_resources[item["name"]]):
                ...
                if show_storage:
                    active_storage = get_active_storage(current_storage, storage_inventory, storage_inventory2)
                    ...
                    if event.key == pygame.K_ESCAPE or event.key == pygame.K_TAB:
                        ...
                        if storage_side == "player":
                            if selected_slot < len(inventory):
                                if len(active_storage) < 12:
                                    active_storage.append(inventory.pop(selected_slot))
                                    if selected_slot >= len(inventory):
                                        selected_slot = max(0, len(inventory) - 1)
                                else:
                                    storage_full_timer = 120
                        else:
                            if selected_slot < len(active_storage):
                                inventory.append(active_storage.pop(selected_slot))
                                if selected_slot >= len(active_storage):
                                    selected_slot = max(0, len(active_storage) - 1)
                    continue
        ...
        if scene == "mars":
            cx = int(active_player.x // CHUNK_SIZE)
            cy = int(active_player.y // CHUNK_SIZE)
            for dcx in range(-1, 2):
                for dcy in range(-1, 2):
                    chunk = (cx + dcx, cy + dcy)
                    if chunk not in visited_chunks:
                        visited_chunks.add(chunk)
                        chunk_x = chunk[0] * CHUNK_SIZE
                        chunk_y = chunk[1] * CHUNK_SIZE
                        if chunk_y + CHUNK_SIZE < 500:
                            continue
                        for _ in range(RESOURCES_PER_CHUNK):
                            for attempt in range(20):
                                rx = random.randint(chunk_x, chunk_x + CHUNK_SIZE - 64)
                                ry = random.randint(max(500, chunk_y), chunk_y + CHUNK_SIZE - 64)
                                resource_data = random.choices(
                                    resource_types,
                                    weights=[r["shans"] for r in resource_types],
                                    k=1
                                )[0]
                                resource = Resource(
                                    rx, ry,
        ...
        if scene == "mars" and not in_rover:
            ...
            if keys[pygame.K_e] and near_resource is not None:
                if pickup_resource != near_resource:
                    ...
                    pickup_progress = 0
                pickup_progress += (1 + player_speed_level * 0.3) / FPS
        ...
        if scene == "mars":
            ...
            if in_rover:
                ...
                target_camera_x = active_player.x - WIDTH // 2.2
                target_camera_y = active_player.y - HEIGHT // 2.2
                ...
                target_camera_x = human.x - WIDTH // 2.1
                target_camera_y = human.y - HEIGHT // 2.1
                camera_x += (target_camera_x - camera_x) * 0.08
                camera_y += (target_camera_y - camera_y) * 0.08
        ...
        if show_storage:
            active_storage = get_active_storage(current_storage, storage_inventory, storage_inventory2)
            draw_storage_window(screen, inventory, active_storage, storage_side, selected_slot, current_storage)
        ...
        if locked_message_timer > 0:
            ...
            pygame.draw.rect(screen, (0, 0, 0), (msg_x - 20, msg_y - 12, message.get_width() + 40, message.get_height() + 24))
        ...
        if show_computer:
            draw_computer_menu(
                screen, computer_menu, computer_selected,
                inventory, storage_inventory,
                rover_speed_level, base_storage_level,
                upgrade_costs, player_speed_level
            )
        ...
        if show_greenhouse_menu:
            draw_greenhouse_menu(screen, greenhouse_selected, crop_recipes, growing_crop, growing_timer, growing_ready)
        ...
        if show_pause_settings:
            draw_pause_settings(
                screen,
                pause_settings_selected,
                show_fps,
                music_volume
            )
        if show_fps:
            fps_font = pygame.font.Font("mars_game/fonts/PressStart2P-Regular.ttf", 12)

            current_fps = int(clock.get_fps())

            fps_text = fps_font.render(f"FPS: {current_fps}", True, (255, 255, 255))

           

            screen.blit(fps_text, (18, 18))
